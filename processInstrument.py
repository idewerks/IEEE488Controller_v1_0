...
def getInstrumentInfo(rm, instrument):
    ...
    # First, lets
    thisInstrument = rm.open_resource(instrument)

    # Note the ID command varies deoending on age of instrument. In particular, IEE488.1 vs IEEE488.2
    ...
    hpibResponse = thisInstrument.query('*IDN?')

    delimiter = ':'
    #if the response is < 6 chars OR it contains a colon, the 488.2 command failed, try a 488.1 command
    if len(hpibResponse) <6 or delimiter in hpibResponse:
        hpibResponse = thisInstrument.query('ID?')
